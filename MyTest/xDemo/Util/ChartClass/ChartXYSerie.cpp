...
//add by 尘中远
//获取所有的x值
unsigned CChartXYSerie::GetAllXPointValue(double* pXValue,unsigned nLength)
{
	if (nLength>m_vPoints.GetPointsCount() || NULL == pXValue)
	{
		return 0;
	}
	unsigned i(0);
	for (i=0;i<nLength;i++)
	{
		pXValue[i] = m_vPoints[i].X;
	}
	return nLength;
}
//获取所有的y值
unsigned CChartXYSerie::GetAllYPointValue(double* pYValue,unsigned nLength)
{
	if (nLength>m_vPoints.GetPointsCount() || NULL == pYValue)
	{
		return 0;
	}
	unsigned i(0);
	for (i=0;i<nLength;i++)
	{
		pYValue[i] = m_vPoints[i].Y;
	}
	return nLength;
}
//获取所有的x，y值
unsigned CChartXYSerie::GetAllXYPointValue(double* pXValue,double* pYValue,unsigned nLength)
{
	if (nLength>m_vPoints.GetPointsCount() || NULL == pXValue || NULL == pYValue)
	{
		return 0;
	}
	unsigned i(0);
